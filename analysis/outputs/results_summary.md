...
_Generated 2026-07-07 19:56. Single always-on condition; no drive-off comparison. Unit = run (10 runs, 8 days, 217 interactions). Two-phase participant design: Phase 1 (first 4 days) had assigned roles (2 obligated feeders, 2 interact-no-feed, rest unconstrained); Phase 2 (last 4 days) unconstrained — used by B10/RQ3._
...
P-values come from cluster-aware models (person-clustered GEE / mixed models) and are Benjamini–Hochberg-corrected **within two pre-declared families** (RQ1/2 behaviour; RQ3 adaptation). **4/4** metrics survive at q<0.05: B3_deficit_pursuit, B10_dose_affinity, B10_role_meal_rate, B10_prior_aff_prioritisation. Implementation checks (B1/B2) carry no inferential p-values by design. Small-n Starving results are still led with effect sizes + CIs rather than NHST (see `bh_corrected_pvalues.csv`).
