...
_Generated 2026-07-07 19:35. Single always-on condition; no drive-off comparison. Unit = run (10 runs, 8 days, 217 interactions). Two-phase participant design: Phase 1 (first 4 days) had assigned roles (2 obligated feeders, 2 interact-no-feed, rest unconstrained); Phase 2 (last 4 days) unconstrained — used by B10/RQ3._
...
| RQ3 | Adaptive affinity reflects real behaviour (role manipulation, dose, downstream use) | **Supported** |
...
- **B3** — Supported: being in a deficit categorically changes what the robot does — it switches on a proactive recovery repertoire that is silent at Full. Face-to-face hunger framing jumps 3% -> 66% (x24); feed-seeking speech acts go 1 -> 20 (deficit-only); co-present feeding pursuit 0.15 -> 0.43 with larger meals (21 -> 31); and the remote channel fires 172 proactive pings in deficit vs 0 at Full. Feeding-pursuit OR 4.9 [2.6,9.5] (person-clustered GEE). This is the action-conversion evidence: the deficit adds goal-directed recovery behaviour in both channels. (Pings/feed-seeking are coded gates = faithful implementation; framing/pursuit/meal-size are emergent measurements of how strongly the deficit reshapes behaviour.)
- **B4** — Supported: when Starving, conversation collapses (turns diff -2.39, Engaged 0.08 vs 0.68; OR 0.03 [0.01,0.14], person-clustered GEE) while feeding pursuit rises (0.54 vs 0.26) — reprioritisation, not disengagement. Starving n=13 (small-n; directional).
...
- **B7** — Supported (the study's headline result): modelled long-run Starving occupancy is median 1.0% [95% 0.2, 3.1%] (run-level block bootstrap; transition-level Poisson bootstrap 1.1% [0.4, 2.3%] agrees) — i.e. the people kept the robot's energy in homeostasis, out of starvation ~98-100% of the time; no absorbing state. This is NOT a self-property of the controller: the transition rates are a record of how the humans actually behaved, so the reading is that human engagement, with demonstrated participation from the drive (see B5), reliably replenished the robot — the HRI loop closes and the solution works. Point est 1.1% is fragile, so we lead with the interval; single condition means the drive's exact causal share in the feeding is not isolated.
- **B8** — Weakened (as a smooth gradient): Engaged-completion declines with severity (rho=-0.16, p=0.016) but the drop is concentrated in Starving ({'Full': 0.69, 'Hungry': 0.67, 'Starving': 0.08}) — Full and Hungry are nearly equal, so this is a step-change at the 25 line, not a smooth Full->Hungry->Starving ramp; turns/energy trends are weak and do not survive covariate adjustment. The step itself is shown directly in B4, not here.
...
- **B10** — Supported: the adaptation tracks real behaviour, not noise. Manipulation check — obligated feeders fed 2.7x the unconstrained rate in Phase 1 [1.2,5.9] and the no-feed pair complied perfectly (0 feeds in Phase 1), with the feeder excess shrinking 0.43x once roles lifted. Core model — +1 SD interaction duration predicts Δaffinity +0.17 [+0.12,+0.21] for unconstrained people, moderated by role (feeders' affinity came from feeding, not chat length) and attenuated in Phase 2; all three dose definitions agree in sign; LOPO-stable. Consequence — prior affinity raises next-day proactive approaches 1.55x per +1 affinity [1.09,2.20] beyond activity. Caveat: 2 people per controlled role (validation, not population inference).
...
P-values come from cluster-aware models (person-clustered GEE / mixed models) and are Benjamini–Hochberg-corrected **within two pre-declared families** (RQ1/2 behaviour; RQ3 adaptation). **5/7** metrics survive at q<0.05: B3_deficit_pursuit, B8_reached_ss4, B10_dose_affinity, B10_role_meal_rate, B10_prior_aff_prioritisation. Implementation checks (B1/B2) carry no inferential p-values by design. Small-n Starving results are still led with effect sizes + CIs rather than NHST (see `bh_corrected_pvalues.csv`).
...
- Long-run Starving occupancy (RQ2-c, headline): bootstrap median 1.0% [95% 0.2, 3.1%] — the people kept the robot's energy in homeostasis, out of starvation ~97%+ of the time (outcome of the working HRI loop, not a controller self-property).
...
- RQ3 (B10): Phase-1 manipulation validated (feeders 2.7x meal rate [1.2,5.9]; no-feed pair 0 feeds); +1 SD interaction duration -> Δaffinity +0.17 [+0.12,+0.21] (role/phase-moderated, dose-definitions agree); prior affinity raises next-day proactive approaches 1.55x [1.09,2.20] — the adaptation reflects real behaviour and changes robot behaviour (2 people per controlled role: validation, not population inference).